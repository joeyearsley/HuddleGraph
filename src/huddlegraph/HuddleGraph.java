...
import java.util.Queue;
    ...
    public class HuddleGraph {
        ...
        public void add(Character vertex) {
            if (neighbourhood.containsKey(vertex)) {
                return;
            }
            ...
            for (int i = 0; i < s.length() - 1; i++) {
                ...
                for (Pair<Character, Integer> neighbor : neighbors) {
                    ...
                    if (neighbor.getFirst().equals(y)) {
                        x = neighbor.getFirst().toString().charAt(0);
         ...
         *
         */
        public int dijkstraDistance(Character start, Character end) {
            ...
            if (start.equals(end)) {
                ...
                while (it.hasNext()) {
                    ...
                    for (Pair<Character, Integer> neighbor : neighbourhood.get(m.getKey())) {
                        ...
                        if (distance.get(end) == 0) {
                            distance.put(end, neighbor.getSecond());
                            //Now has a value so check that the potential new one is lower
                        } else if (neighbor.getFirst().equals(end)
                                && (newVal < distance.get(end))) {
                            distance.put(end, newVal + neighbor.getSecond());
                        }
                    }
                }
            }
            //return the distance it takes to get to the end vertex
            return Integer.parseInt(distance.get(end).toString());
        }
